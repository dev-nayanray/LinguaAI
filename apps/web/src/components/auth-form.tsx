 ...
 * the whole form's result (login failure, "check your email"), distinct
 * from `FormField`'s own per-field error slot — which DESIGN_SYSTEM.md §4's
 * Forms category doesn't define a component for.
 */
...
export function FormSuccess({ message }: { message: string | null }) {
  ...
  return (
    <p className="rounded-md border border-success-text bg-surface px-3 py-2 text-sm text-success-text">
      {message}
    </p>
  );
}
