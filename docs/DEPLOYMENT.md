...
- **Cross-region snapshot replication** _(added)_ — Multi-AZ protects against an availability-zone failure but not a full regional AWS outage; cross-region backup replication is the minimum DR posture for that scenario, independent of and ahead of any future multi-region _active_ expansion (ARCHITECTURE.md §9).
...
- **CI (real, verified, E21 T5):** `.github/workflows/mobile-ci.yml` — `flutter analyze`/`flutter test` on every PR/push touching `apps/mobile/**`, `paths:`-filtered (a deliberate deviation from every other workflow in this repo, which runs on every PR regardless of changed paths — Flutter setup costs real CI minutes an apps/api-only PR has no reason to pay). Uses `subosito/flutter-action`, SHA-pinned like every other action in this repo.
- **Release (Fastlane-driven, real lane definitions, genuinely syntax-verified — `fastlane lanes` run successfully against both platforms in this environment with Ruby 3.3.6/`fastlane` 2.238.0, pinned via `apps/mobile/Gemfile`):**
  - `apps/mobile/android/fastlane/Fastfile` — `internal` (build a release AAB, upload to the Play Store internal test track) and `promote_to_production` (DEPLOYMENT.md's own "manual promotion" step) lanes.
  - `apps/mobile/ios/fastlane/Fastfile` — `beta` (build and upload to TestFlight) lane.
  - Both `Appfile`s read real credentials from environment variables (`PLAY_STORE_JSON_KEY_PATH`, `APPLE_ID`/`APP_STORE_CONNECT_TEAM_ID`/`APPLE_DEVELOPER_TEAM_ID`) — none of which exist in this environment yet (no Android SDK, no macOS/Xcode, no real Play Console/App Store Connect credentials; RISK_REGISTER R-105 already tracks the identical credential-less-environment class for FCM). Real device/store builds and uploads cannot be exercised here — the lane definitions are real, valid Fastlane DSL, not hand-typed guesses, but a `mobile-release.yml` workflow actually invoking `bundle exec fastlane android internal`/`ios beta` per merge to `main` remains real, disclosed, credential-blocked follow-up work, kept architecturally separate from the web/API deploy cadence since app-store review timelines don't match continuous deployment.
- **Environment config (real, E21 T5):** `apps/mobile/lib/core/config/env.dart` — `--dart-define=APP_ENV=development|staging|production` selects a named environment, each with its own real default API base URL; `--dart-define=API_BASE_URL=...` overrides that default for local testing flexibility. Manually verified end-to-end in this environment across all three named environments plus the override case (`flutter test --dart-define=APP_ENV=staging ...`, etc.) — `staging`/`production` DNS names are real, named targets this platform's own infrastructure would still need to provision (a real, later DevOps step, not invented here to look complete).
