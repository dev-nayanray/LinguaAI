...
- **Phase 3 (blocked on a backend branch merging to `main` first, corrected from this document's earlier draft):** speaking/pronunciation/listening/reading/writing practice (E10–E13), exam prep (E19), certificates (E20), `/progress` (gamification, E14), `/billing` (E15), `/settings/notifications` (E16) — none of these modules exist under `apps/api/src/modules/` on `main` at all (confirmed by direct search, not just the ROADMAP.md table, which shows all of them as epic-complete but doesn't reflect what's actually merged). Every one of them is real, but only on its own separate, unmerged epic branch — the same situation as E14/E15/E16/E21. This document's earlier draft assumed E10–E13/E19/E20 were already on `main`; they are not.
