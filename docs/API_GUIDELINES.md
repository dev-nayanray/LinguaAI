...
## 13. Server-Sent Events conventions (ADR-003, ADR-033)

For a one-directional generation/progress stream over an existing REST endpoint (WebSocket per §9 remains reserved for genuinely bidirectional real-time flows):

- **Not NestJS's `@Sse()` decorator** when the endpoint needs a request body — that decorator forces `GET`. Managed manually instead: set `Content-Type: text/event-stream`/`Cache-Control: no-cache`/`Connection: keep-alive`, call `flushHeaders()`, then write each event as its own `data: <json>\n\n` line (`response.write(...)`), ending the response (`response.end()`) once the source completes.
- **One JSON value per `data:` line**, discriminated on the payload's own `type` field — no named SSE `event:` fields. A client needs only one listener/stream-reader and switches on the parsed payload's `type`, matching this platform's own established discriminated-union convention (e.g. `loginResponseSchema`) rather than inventing a second discrimination mechanism at the transport layer.
- **A failure _before_ the first event** is a normal HTTP error response (the relevant status code + the standard `{ error: { code, message } }` envelope, §3) — nothing has been written to the response yet.
- **A failure _after_ streaming has begun** cannot change the HTTP status (headers are already sent as `200`) — it is instead reported as a `{ type: 'error', message }` event within the stream itself, and the response still ends normally. A source that has already yielded real content and then fails does not retry/fail over onto a different source, for the same reason `RouterService.stream()`'s own failover logic doesn't: a retry would duplicate already-delivered content with no way to reconcile the two partial results.
- **First real implementation**: E5 T10 (ADR-033) — `services/ai-engine/src/agent-sessions/agent-sessions.controller.ts`'s `POST /v1/agent-sessions/:id/messages`.
