...
| Forms                        | Text input, textarea, select, checkbox, radio, switch, combobox, all with label/help-text/error-text slots accepting a pre-parsed `error?: string` prop — the caller resolves `packages/validation` Zod errors to a string before passing them down (E3 T5/ADR; `packages/ui` takes no direct dependency on `packages/validation`, per §6 of the E3 design document)                                                                                                                 |
