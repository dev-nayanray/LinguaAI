...
- `PronunciationScore` — phoneme-level scoring results tied to an `AIMessage` or a `PronunciationLabAttempt`, still a polymorphic `(sourceType, sourceId)` pointer with no DB-level FK (modeled that way in T5). **Gap closed (E11 T2):** `PronunciationLabAttempt` — a learner's recorded attempt at a known target phrase, scored via `services/speech-service`'s `PronunciationProvider` (ADR-049) and persisted by `apps/api` directly (ADR-050) — is now a real entity, closing this document's own previously-flagged gap.
