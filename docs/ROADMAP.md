...
| E6  | [AI Language Assessment Engine](epics/E6-ai-language-assessment-engine.md)               | L          | E4, E5 — **implementation in progress (T1–T6 done, 2026-08-07): AssessmentItem item bank (ADR-037); apps/api AssessmentModule attempt lifecycle, adaptive item selection, objective scoring (ADR-038); CEFR banding/confidence + ProficiencyLevel/History writes; ai-engine AssessmentScoringService for Writing-skill AI scoring (ADR-039); apps/api↔ai-engine REST contract for Writing scoring (E5 T9–T12 merged in to unblock this); `assessment.attempt.completed` event emission, user-initiated re-assessment flow, low-confidence retake-offer contract, by explicit user direction rather than an independent Architecture Gate review; §12's gate sign-off log remains unchecked, not yet formally closed** |
