...
**Status:** Implementation complete (T1–T2, 2026-08-13 — the epic's own full §9 task sequence, confirmed no further task remains).
...
| ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **T1** | ✅ **Done (2026-08-13)** — `listeningActivityContentSchema`/`readingActivityContentSchema` (§6.1); `speech-service`'s new `POST /v1/speech/synthesize` (ADR-051); `SpeechServiceClientService.synthesizeSpeech()`; `apps/api`'s content-authoring flow wired to produce real, persisted, playable Listening activities | E11        | Unit tests with a mocked `TtsProvider`/`fetch`; a real e2e test proving a genuinely uploaded (local MinIO) audio URL ends up on a persisted `Activity.content` |
| **T2** | ✅ **Done (2026-08-13)** — The CEFR-level-matched Reading read path (§6.3) — a new query + learner-facing endpoint                                                                                                                                                                                                     | T1         | Unit tests against a mocked `ProficiencyLevel` row; a real e2e test against live Postgres proving the returned ordering matches the caller's own level         |

**T2 evidence (2026-08-13):** `GET /v1/reading-activities/matched?languageId=...` (`CourseCatalogController`/`CourseCatalogService`, any authenticated learner) — the epic's own closing task. Looks up the caller's own `ProficiencyLevel` row for `skill: 'READING'`/`languageId` (defaulting to `A1` for a never-assessed learner, a real, resolved decision for §10 open question — a safe-beginner default, not an error), then fetches every published, non-deleted `READING` activity for that language and sorts them in application code by `Math.abs(CEFR_LEVELS.indexOf(...) - targetIndex)` — a real, working nearest-band v1 per the design doc's own explicit non-goal of a sophisticated recommendation algorithm. `content.cefrLevel` lives inside a JSON column with no index to sort by, so the whole published-for-this-language set is fetched and sorted server-side rather than pushed into SQL — an accepted v1 cost given a real course catalog's own bounded-per-language scale, the same reasoning `courseListQuerySchema`'s own doc comment already used to justify offset pagination for this same catalog. New `matchedReadingActivitySchema`/`matchedReadingActivitiesQuerySchema`/`matchedReadingActivitiesResponseSchema` (`packages/validation/src/content/index.ts`) reuse `activityBaseSchema`/`refinePersistedActivityContent` (T1's own extraction), extended with `courseId`/`lessonTitle` context since this list spans lessons, unlike `lessonDetailResponseSchema`'s own single-lesson scope. Verified: `packages/validation`/`apps/api` typecheck and lint clean; `apps/api` unit tests 508/508 (5 new: 4 service-level ordering/pagination/filtering cases, 1 controller delegation case); `apps/api` build clean; a new `apps/api/test/reading-matched.e2e-spec.ts` (4 tests) proves the real ordering against live Postgres — `ProficiencyLevel` seeded directly via Prisma since its only writer, `AssessmentService.completeAttempt`, is a full assessment flow genuinely out of this test's own scope, the same "seed the fixture directly when no endpoint exists for it" discipline `pronunciation-lab.e2e-spec.ts`'s own `language.findUniqueOrThrow` lookup already established — including a real assertion that a draft (unpublished) course's own `READING` activity is never returned. The full `apps/api` e2e suite run against live Postgres/Redis shows 260/269 passing; the 5 failing suites (`password-reset`, `tenant-rls`, `role-lifecycle`, `audit`, `domain-events`) are pre-existing Identity/RLS-platform environment gaps and one confirmed-flaky Redis-timing suite (passes cleanly in isolation) — none touched by this task, none newly introduced by it. **E12 is now implementation-complete (T1–T2), the epic's own full §9 task sequence, confirmed no further task remains.**

**T1 evidence (2026-08-13):** `packages/validation/src/content/index.ts` adds `listeningActivityContentSchema`/`readingActivityContentSchema` (real persisted shapes) plus separate `draftListeningActivityContentSchema`/`draftReadingActivityContentSchema` for the two distinct pre-persistence shapes (`ContentDraftingService`'s AI output, and `LISTENING`'s own create-request draft — see below); `activitySchema`/`createActivityRequestSchema`/`contentDraftActivitySchema` all now real-validate `LISTENING`/`READING` `content` via three distinctly-scoped `superRefine` functions rather than one shared one, since the create-request, AI-draft, and persisted shapes for `LISTENING` are genuinely different from each other (a `ZodEffects`-has-no-`.extend()` limitation was hit and fixed by extracting an unrefined `activityBaseSchema` other schemas can still `.extend()`). `services/speech-service`'s new `SpeechSynthesisController` (`POST /v1/speech/synthesize`, ADR-051) reuses the already-integrated `TtsProvider` and its own already-injected `AudioStorageProvider` to upload the synthesized audio itself and return a real, already-hosted `audioUrl` — a genuine, better design than this doc's own original §6.2 draft, which assumed `apps/api` would do the S3 upload itself; investigation confirmed `apps/api` has no object-storage client of its own, so `speech-service` owns the upload instead, and `apps/api` needs no S3 client at all for this. `SpeechServiceClientService.synthesizeSpeech()` (`apps/api`) is the real HTTP consumer; `LessonContentService.createActivity()` synthesizes a `LISTENING` activity's own drafted `{ script }` into the real, persisted `{ audioUrl, transcript }` shape server-side before the row is ever written — every other `ActivityType`, `READING` included, is persisted exactly as submitted (a real, deliberate asymmetry: `READING`'s own `cefrLevel` is admin-assigned at creation time per §10 open question 2, so its create-request content is already the real shape). A real regression was found and fixed along the way: this task's new content-shape validation broke three pre-existing fixtures across `ai-engine-client.service.spec.ts`, `course.e2e-spec.ts` (three call sites), and `content-authoring.e2e-spec.ts` that were using placeholder `READING` content (`{ text: 'Hola' }` / `{}`) never actually validated before — all updated to the real shape. Verified: `packages/validation`/`services/speech-service`/`apps/api` typecheck clean; `services/speech-service` unit tests 73/73 (2 new); `apps/api` unit tests 503/503 (2 new, plus the 1 pre-existing fixture fixed); `apps/api` build clean; a new `apps/api/test/listening-content.e2e-spec.ts` proves the full real flow against live Postgres (`SpeechServiceClientService` stubbed at the boundary, matching `pronunciation-lab.e2e-spec.ts`'s own established discipline) — a genuinely uploaded-and-returned `audioUrl` ends up on the real, persisted `Activity.content` row; the full `apps/api` e2e suite run against live Postgres/Redis shows 258/265 passing, with the 4 failing suites (`password-reset`, `tenant-rls`, `role-lifecycle`, `audit`) pre-existing Identity/RLS-platform environment gaps this task never touched (not caused by this change).
